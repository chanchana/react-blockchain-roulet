...
export const DealerBalanceManagement = ({ handleDealerDeposit, handleDealerWithdraw, dealerBalance, handleBeDealer, handleResignFromDealer }) => {
  ...
  const handleAction = () => {
    if (mode === 'Deposit') {
      handleDealerDeposit(value)
    } else if (mode === 'Withdraw') {
      handleDealerWithdraw(value)
    }
    setValue(0)
  }

  const mode = useMemo(() => tabValue === 0 ? 'Deposit' : 'Withdraw', [tabValue])
        ...
        <DialogContent style={{color: 'black'}}>
          ...
          {/* <Paper square> */}
            ...
            <Box margin="1rem 0" textAlign="center">
              ...
              <Box color="gray">{mode}</Box>
              <Box fontSize="1.2rem" fontWeight="bold" marginTop="0.2rem" >{value} ETH</Box>
              <Box color="gray">Dealer Balance: {dealerBalance}</Box>
            </Box>
          {/* </Paper> */}
            {/* <TextField
              autoFocus
